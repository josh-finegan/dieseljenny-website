// lib/config.ts

export const bio = "Diesel Jenny launches their debut release, a double A-side as part of their new white label series. The ferociously rungle two-tracker is the first trip out under a new moniker from veteran Pōneke-based producer Josh Finegan (fka ABG). With a back catalogue of break-heavy club tools and blown-out emotional heaters via underground mainstays Big Fresh Collective and Bankrupt Records, DJEN001 marks both a clean start and a continuation.";

export const phrases = [
    '‘NEVER DONE THIS TO ME BEFORE!’',
    "'LIKE A RAT UP A DRAINPIPE!'",
    "'DAMN DIESEL!'",
    "'SANJAY'S SELECTIONS - THE OFFICIAL SPONSOR OF DIESEL JENNY'"
];

export const socialLinks = {
    instagram: "https://www.instagram.com/diesel__jenny/",
    bandcamp: "https://dieseljenny.bandcamp.com/",
    facebook: "https://www.facebook.com/DieselJenny/",
    youtube: "https://www.youtube.com/@DieselJenny",
    tiktok: "https://www.tiktok.com/@dieseljenny"
};

export const contactEmail = "dieseljenny@proton.me";

export const latestUpdates = "DJEN001 OUT NOW VIA BANDCAMP. REPRESS COMING SOON. NEXT WHITE LABEL SERIES IN THE WORKS.";

export const embeds = {
    bandcamp: "https://bandcamp.com/EmbeddedPlayer/album=3807677691/size=large/bgcol=333333/linkcol=ffffff/tracklist=false/transparent=true/",
    youtube: "https://www.youtube.com/embed/6I3w-xRergs?si=iUfbDbHhb789Lh5Q&modestbranding=1&controls=0"
};

export const audioStings = [
    '/assets/AudioStings/Jamaican Horn Siren.mp3',
    '/assets/AudioStings/Jeeeers.mp3',
    '/assets/AudioStings/Oi who the fak.mp3',
    '/assets/AudioStings/Bro is that roti chanai.mp3',
    '/assets/AudioStings/peopledonothing-successvocals.mp3'
];
